...
(defun handle_info (fd clients ignore cleanup)
  ...
  (receive ((tuple 'udp fd ip port msg)
	    ...
	    (if (andalso (== (car cleanup) 'true)
		 (> (: timer now_diff (now) (cdr cleanup)) 5000000)) ; 5 sec
		 (handle_info fd clients () (cons 'false 0))
	      (if (: lists any (lambda (x) (== x (cons ip port))) ignore)
		;(: io format '"Ignored: ~p~n" (list ignore))
		(handle_info fd clients ignore cleanup)
		(let ((pid (spawn_link (lambda () (process_msg msg)))))
		  ;(: io format '"Processing message ~p~n" (list msg))
		  (handle_info fd (cons (tuple pid (cons ip port)) clients) ignore cleanup)))))
	   ((tuple _ pid 'normal)
	   ...
	   ((tuple _ pid _)
		...
		(handle_info fd newclients newignore (cons 'true (now)))
		(handle_info fd newclients newignore cleanup)))))
    ;(tuple 'noreply state))
)
(defun start_link (args)
  ;(if (do-tests)
	...
	(handle_info socket () () (cons 'false 0))))
    ;#(error tests-failed))
)
